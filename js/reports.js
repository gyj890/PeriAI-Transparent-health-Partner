...
async function buildAndGoReport() {
  await buildReport();
  goTo('reports');
}
...
function buildRisks(logged) {
  ...
  if (ids.includes('chest_pain')) risks.push({ level: 'high', icon: '!', text: 'Chest pain reported — cardiac evaluation and ECG strongly recommended.' });
  if (ids.includes('breathless')) risks.push({ level: 'high', icon: '!', text: 'Shortness of breath — rule out cardiac and pulmonary causes.' });
  if (ids.includes('swelling')) risks.push({ level: 'mod', icon: '~', text: 'Leg swelling noted — may indicate fluid retention or cardiac involvement.' });
...
function buildDiscussion(logged) {
  ...
  if (ids.includes('chest_pain') || ids.includes('breathless')) pts.push('Cardiac evaluation — ECG, stress test, and cardiology referral recommended');
  ...
  if (ids.includes('dizziness')) pts.push('Orthostatic hypotension assessment — blood pressure lying vs standing');
  if (ids.includes('swelling')) pts.push('Fluid retention evaluation — echocardiogram and BNP level recommended');
  if (ids.includes('nausea')) pts.push('Unexplained nausea — rule out cardiac origin especially if with chest symptoms');
...
function copyRpt(i) {
  ...
  const lines = [
    ...
    '', 'Generated by PeriAI CardioAI Companion. Not a medical diagnosis.'
  ];
